...
public:
 ...
 // LoadNxCore version of NxCoreAPI if successful, 0 on failure
  unsigned int LoadNxCore(const char* cszNxCoreLib)
  {
	...
	wchar_t wtext[20];
	mbstowcs(wtext, cszNxCoreLib, strlen(cszNxCoreLib) + 1);//Plus null
	hLib = ::LoadLibrary(wtext);
	//End Insert by SAH
	//hLib = ::LoadLibrary(cszNxCoreLib); //Original
	//hLib = ::LoadLibrary((LPCWSTR)L"NxCoreAPI.dll"); //SAH
	if( hLib ) {
